...
class AttentiveEraser:
    ...
    def __init__(self, num_steps=200, device=None):
    ...
    def __call__(self, image, mask):
        ...
        paste_mask = mask.convert("L").point(lambda v: 255 if v > 0 else 0)
        # 膨胀一圈并做轻微羽化，确保重绘区域覆盖到原边缘之外
        paste_mask = paste_mask.filter(ImageFilter.MaxFilter(15)).filter(ImageFilter.GaussianBlur(radius=1))
        out_img = copy(image)
