...
class OrientationVisualizeTest(Visualizer):
    KEYS = ['image', 'centers', 'centers-est', 'pred', 'sigma', 'seed', 'gt-diff', 'centerdir_gt',
            'mean', 'variance', 'mixture', 'direction-field', 'orientation-field']
    ORIENTATION_OFFSET = 3
    VARIANCE_COLORMAP = cv2.COLORMAP_PLASMA
    MIXTURE_COLORMAP = cv2.COLORMAP_VIRIDIS

    def __init__(self, keys=(), show_rot_axis=(True,True,True), axis_from_eulers=False, show_regressed_maps=True,
                 show_gt_maps=True, plot_rank_number=False, bgr_axis=False, show_confidence_score=False,
                 direction_field_stride=10, direction_field_arrow_length=18, direction_field_color=(0, 0, 0),
                 direction_field_alpha=0.55,
                 orientation_field_stride=8, orientation_field_arrow_length=12, orientation_field_colors=None,
                 orientation_field_alpha=0.7,
                 **kwargs):
        ...
        self.direction_field_stride = direction_field_stride
        self.direction_field_arrow_length = direction_field_arrow_length
        self.direction_field_color = tuple(direction_field_color)
        self.direction_field_alpha = direction_field_alpha
        self.orientation_field_stride = orientation_field_stride
        self.orientation_field_arrow_length = orientation_field_arrow_length
        self._orientation_field_colors = orientation_field_colors
        self.orientation_field_alpha = orientation_field_alpha
        ...
        if self._orientation_field_colors is None:
            self.orientation_field_colors = self.color_rot_axis
        else:
            self.orientation_field_colors = list(self._orientation_field_colors)
        del self._orientation_field_colors
    ...
    def _prepare_probabilistic_maps(self, variance_maps, mixture_weights, mean_maps, orientation_dims):
        variance_images = []
        mixture_images = []
        mean_images = []

        if orientation_dims <= 0:
            return variance_images, mixture_images, mean_images

        channel_end = None

        def prepare_tensor(tensor):
            nonlocal channel_end
            if tensor is None:
                return None, None
            if tensor.dim() != 5:
                return None, None
            ...
            if channel_end is None:
                channel_end = min(tensor.shape[0], self.ORIENTATION_OFFSET + 2 * orientation_dims)
            if channel_end <= self.ORIENTATION_OFFSET:
                return None, None
            return tensor[self.ORIENTATION_OFFSET:channel_end], channel_end

        variance_tensor, _ = prepare_tensor(variance_maps)
        mixture_tensor, _ = prepare_tensor(mixture_weights)
        mean_tensor, _ = prepare_tensor(mean_maps)

        if variance_tensor is not None:
            variance_per_component = variance_tensor.mean(dim=0)
            variance_images = [variance_per_component[k].detach().cpu() for k in range(variance_per_component.shape[0])]

        if mixture_tensor is not None:
            mixture_per_component = mixture_tensor.mean(dim=0)
            mixture_images = [mixture_per_component[k].detach().cpu() for k in range(mixture_per_component.shape[0])]

        if mean_tensor is not None:
            mean_per_component = mean_tensor.mean(dim=0)
            mean_images = [mean_per_component[k].detach().cpu() for k in range(mean_per_component.shape[0])]

        return variance_images, mixture_images, mean_images

    def _build_dense_field_plot_fn(self, sin_maps, cos_maps, *, stride, arrow_length,
                                   colors, show_axes, alpha):
        sin_np = sin_maps.detach().cpu().numpy()
        cos_np = cos_maps.detach().cpu().numpy()

        step = max(1, int(round(stride)))
        arrow_len = max(1.0, float(arrow_length))
        colors = list(colors) if colors else [(0, 0, 0)]
        if show_axes is None:
            show_axes = (True,) * len(sin_np)
        ...
        def plot_fn(img):
            ...
            field_h, field_w = sin_np.shape[-2], sin_np.shape[-1]
            if field_h == 0 or field_w == 0:
                return overlay

            scale_y = img_h / field_h
            scale_x = img_w / field_w

            for axis_idx in range(len(sin_np)):
                if axis_idx >= len(show_axes) or not show_axes[axis_idx]:
                    continue

                color = colors[axis_idx % len(colors)]
                sin_field = sin_np[axis_idx]
                cos_field = cos_np[axis_idx]

                for fy in range(0, field_h, step):
                    for fx in range(0, field_w, step):
                        s_val = float(sin_field[fy, fx])
                        c_val = float(cos_field[fy, fx])
                        norm = math.hypot(c_val, s_val)
                        ...
                        px = int(round((fx + 0.5) * scale_x))
                        py = int(round((fy + 0.5) * scale_y))

                        end_x = int(round(px + dir_x * arrow_len))
                        end_y = int(round(py + dir_y * arrow_len))

                        px = int(np.clip(px, 0, img_w - 1))
                        py = int(np.clip(py, 0, img_h - 1))
                        end_x = int(np.clip(end_x, 0, img_w - 1))
                        end_y = int(np.clip(end_y, 0, img_h - 1))

                        cv2.arrowedLine(overlay, (px, py), (end_x, end_y), color=color, thickness=1, tipLength=0.3)

            if alpha is not None:
                alpha_clamped = np.clip(alpha, 0.0, 1.0)
                return cv2.addWeighted(img, 1 - alpha_clamped, overlay, alpha_clamped, 0)
            return overlay

        return plot_fn

    def _build_dense_orientation_plot_fn(self, sin_maps, cos_maps):
        show_axes = self.show_rot_axis if self.show_rot_axis is not None else None
        return self._build_dense_field_plot_fn(
            sin_maps, cos_maps,
            stride=self.orientation_field_stride,
            arrow_length=self.orientation_field_arrow_length,
            colors=self.orientation_field_colors,
            show_axes=show_axes,
            alpha=self.orientation_field_alpha
        )

    def _build_dense_direction_plot_fn(self, sin_map, cos_map):
        sin_maps = sin_map.unsqueeze(0)
        cos_maps = cos_map.unsqueeze(0)
        return self._build_dense_field_plot_fn(
            sin_maps, cos_maps,
            stride=self.direction_field_stride,
            arrow_length=self.direction_field_arrow_length,
            colors=[self.direction_field_color],
            show_axes=(True,),
            alpha=self.direction_field_alpha
        )
    ...
    def visualize_opencv(self, sample, result, pred_list, pred_score, pred_poly_mask, pred_gt_match, difficult, base, save_dir, plot_bbox_all=False):
        ...
        OFFSET = self.ORIENTATION_OFFSET
        ...
        fig_direction_field = None
        if sin_pred is not None and cos_pred is not None:
            dense_direction_plot = self._build_dense_direction_plot_fn(
                sin_pred.detach(), cos_pred.detach()
            )
            def direction_overlay(img):
                img = dense_direction_plot(img)
                return plot_predictions(img, gt=True, bbox=False)
            fig_direction_field = self.display_opencv(
                im,
                'direction-field',
                plot_fn=direction_overlay
            )

        fig_orientation_field = None
        if orientation_dims > 0:
            dense_field_plot = self._build_dense_orientation_plot_fn(
                orientation_sin_pred.detach(), orientation_cos_pred.detach()
            )
            def orientation_overlay(img):
                img = dense_field_plot(img)
                return plot_predictions(img, gt=True, bbox=False)
            fig_orientation_field = self.display_opencv(
                im,
                'orientation-field',
                plot_fn=orientation_overlay
            )

        variance_images, mixture_images, mean_images = self._prepare_probabilistic_maps(
            result.get('variance_maps'),
            result.get('mixture_weights'),
            result.get('mean_maps'),
            orientation_dims
        )

        if len(mean_images) > 0:
            fig_mean = self.display_opencv(
                mean_images,
                'mean',
                plot_fn=partial(plot_predictions, bbox=plot_bbox_all),
                image_colormap=[cv2.COLORMAP_PARULA] * len(mean_images)
            )
        if len(variance_images) > 0:
            fig_variance = self.display_opencv(
                variance_images,
                'variance',
                plot_fn=partial(plot_predictions, bbox=plot_bbox_all),
                image_colormap=[self.VARIANCE_COLORMAP] * len(variance_images)
            )
        if len(mixture_images) > 0:
            fig_mixture = self.display_opencv(
                mixture_images,
                'mixture',
                plot_fn=partial(plot_predictions, bbox=plot_bbox_all),
                image_colormap=[self.MIXTURE_COLORMAP] * len(mixture_images)
            )
        ...
        if save_dir is not None:
            ...
            if 'fig_direction_field' in locals() and fig_direction_field is not None:
                cv2.imwrite(os.path.join(save_dir, '%s_2.direction-field.png' % base), fig_direction_field)
            if 'fig_orientation_field' in locals() and fig_orientation_field is not None:
                cv2.imwrite(os.path.join(save_dir, '%s_3.orientation-field.png' % base), fig_orientation_field)
            if 'fig_mean' in locals():
                cv2.imwrite(os.path.join(save_dir, '%s_4.mean.png' % base), fig_mean)
            if 'fig_variance' in locals():
                cv2.imwrite(os.path.join(save_dir, '%s_5.variance.png' % base), fig_variance)
            if 'fig_mixture' in locals():
                cv2.imwrite(os.path.join(save_dir, '%s_6.mixture.png' % base), fig_mixture)
