...
from criterions.center_direction_loss import (
    CenterDirectionLoss,
    gaussian_nll_loss,
    gmm_nll_loss,
)
...
class OrientationLoss(nn.Module):
    def __init__(self, model, orientation_args=dict(), **center_dir_args):
        ...
        self.probabilistic = center_dir_args.get('probabilistic', False)
        self.mixture_model = center_dir_args.get('mixture_model', False)
        self.num_mixtures = center_dir_args.get('num_mixtures', 1)
        ...
        # --- PGF-Net MODIFICATION START ---
        # 这里的num_vector_fields现在代表均值(mu)场的数量
        # 断言仍然有效
        # --- PGF-Net MODIFICATION END ---
    ...
    def forward(self, prediction, sample, centerdir_responses=None, centerdir_gt=None, ignore_mask=None,
        ...
        rot_dim = 3 if self.enable_6dof else 1
        sin_slice = slice(3, 3 + rot_dim)
        cos_slice = slice(3 + rot_dim, 3 + 2 * rot_dim)

        if self.probabilistic and self.mixture_model:
            mu_sin_components = mu_all[:, sin_slice]
            mu_cos_components = mu_all[:, cos_slice]
            log_var_sin_components = log_var_all[:, sin_slice]
            log_var_cos_components = log_var_all[:, cos_slice]
            alpha_sin_components = alpha_logits_all[:, sin_slice]
            alpha_cos_components = alpha_logits_all[:, cos_slice]

            prediction_sin_orientation = torch.sum(mu_sin_components * alpha_prob_all[:, sin_slice], dim=2).unsqueeze(2)
            prediction_cos_orientation = torch.sum(mu_cos_components * alpha_prob_all[:, cos_slice], dim=2).unsqueeze(2)
        else:
            prediction_sin_orientation = centerdir_vectors[:, sin_slice].unsqueeze(2)
            prediction_cos_orientation = centerdir_vectors[:, cos_slice].unsqueeze(2)

        if self.probabilistic and not self.mixture_model:
            log_var_sin_orientation = log_var_vectors[:, sin_slice].unsqueeze(2)
            log_var_cos_orientation = log_var_vectors[:, cos_slice].unsqueeze(2)
        ...
        loss_sin_terms = [loss_zero_init() for _ in range(ROT_DIM)]
        loss_cos_terms = [loss_zero_init() for _ in range(ROT_DIM)]
        loss_confidence_terms = [loss_zero_init()] if self.regress_confidence_score else []
        ...
        if self.enable_orientation_loss:
            ...
            with torch.no_grad():
                ...
                if mask_weights.dim() == 4 and mask_weights.shape[1] == 1:
                    mask_weights = mask_weights[:, 0]
                elif mask_weights.dim() != 3:
                    raise ValueError(f"Unsupported mask_weights shape {mask_weights.shape}")

            if self.probabilistic and self.mixture_model:
                sin_loss_map = gmm_nll_loss(mu_sin_components, log_var_sin_components, alpha_sin_components, gt_sin_orientation).squeeze(2)
                cos_loss_map = gmm_nll_loss(mu_cos_components, log_var_cos_components, alpha_cos_components, gt_cos_orientation).squeeze(2)
            elif self.probabilistic and not self.mixture_model:
                sin_loss_map = gaussian_nll_loss(prediction_sin_orientation, log_var_sin_orientation, gt_sin_orientation).squeeze(2)
                cos_loss_map = gaussian_nll_loss(prediction_cos_orientation, log_var_cos_orientation, gt_cos_orientation).squeeze(2)
            else:
                sin_loss_map = self.regression_loss_fn(prediction_sin_orientation, gt_sin_orientation).squeeze(2)
                cos_loss_map = self.regression_loss_fn(prediction_cos_orientation, gt_cos_orientation).squeeze(2)

            if self.no_instance_loss:
                for idx in range(ROT_DIM):
                    loss_sin_terms[idx] += torch.sum(mask_weights * sin_loss_map[:, idx:idx+1], dim=reduction_dims)
                    loss_cos_terms[idx] += torch.sum(mask_weights * cos_loss_map[:, idx:idx+1], dim=reduction_dims)

                if self.regress_confidence_score:
                    conf_loss_map = self.regression_loss_fn(prediction_confidence_score, gt_confidence_score).squeeze(2)
                    loss_confidence_terms[0] += torch.sum(mask_weights * conf_loss_map, dim=reduction_dims)

            else:
                for b in range(mask_weights.shape[0]):
                    mask_weights_b = mask_weights[b]
                    fg_mask_weights = mask_weights_b[fg_mask[b]]
                    for rot_dim in range(ROT_DIM):
                        loss_sin_terms[rot_dim][b] += torch.sum(
                            fg_mask_weights * sin_loss_map[b, rot_dim][fg_mask[b]])
                        loss_cos_terms[rot_dim][b] += torch.sum(
                            fg_mask_weights * cos_loss_map[b, rot_dim][fg_mask[b]])

                    if self.regress_confidence_score:
                        conf_loss_map = self.regression_loss_fn(prediction_confidence_score[b], gt_confidence_score[b]).squeeze()
                        loss_confidence_terms[0][b] += torch.sum(fg_mask_weights * conf_loss_map[fg_mask[b]])

        loss_sin_orientation = [w_orientation * l for l in loss_sin_terms]
        loss_cos_orientation = [w_orientation * l for l in loss_cos_terms]

        loss_confidence_score = [w_confidence_score * l for l in loss_confidence_terms]
